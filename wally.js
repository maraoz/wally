...
function hdrdb_load()
{
    ...
    if (fs.existsSync(hdrdb_fn)) {
        hdrdb.readFile(hdrdb_fn);
        console.log("### Headers DB read. Height is: " + hdrdb.size() );
    }
}
...
console.log("### ADDING..." + hashStr + " @ " +  hdrdb.cached_size );
	try {
		hdrdb.add(block);
	} catch(e) {
		var height = hdrdb.cached_size;
...
console.log("About to add headers: " + headers.length );
...
function main()
{
	...
	if (! program.pass) {
		console.error("No passphrase given (use -P <passphrase>)");
		list_commands();
		return;
	}
